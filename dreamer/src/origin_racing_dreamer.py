...
class RacingDreamer:
    ...
    def _preprocess_lidar(self, scan):
        ...
        embed = np.clip(scan, min_range, max_range)
        # Step 2: normalize embed ranges in 0, 1
        embed = (embed - min_range) / (max_range - min_range) - 0.5
        embed = np.expand_dims(embed, 0).astype('float32')
        return embed
